...
def extract_golden(pcw: Sheet, laser: Sheet) -> dict:
    ...
    *input* -- what the engine needs to reproduce the case. The workbook's own
    ...
    # F33 "Pallet Change Time Hrs/Part" is 60/100/3600, not 60/parts_per_sheet
    # -- the reading that reproduces the oracle. It is also numerically equal
    # to F32, the rapid factor, so one saved quote cannot rule out a
    # copy-paste. Flagged rather than assumed.
    if abs((laser.num(33, "F") or 0) - (laser.num(32, "F") or -1)) < 1e-12:
        note("laser worksheet", "F33", "pallet_change_hrs_per_part",
             "F33 (pallet change) equals F32 (rapid factor) exactly in this "
             "save, so the engine's reading -- one pallet change per 100-part "
             "batch, 60/100 s per part -- cannot be distinguished from a "
             "copy-paste of the cell above it. The reading reproduces §9 and "
             "60/parts_per_sheet does not (4.9% out). Ask the owner: is a "
             "pallet change costed once per 100-part run, or once per sheet? "
             "REQUIREMENTS §10 q10.")
