...
### 1.2 Calc engine — types and material module ⏱ 1 h
*Prompt to Claude:*
> Read CLAUDE.md, REQUIREMENTS §3, §5.1, §5.7 and §9, and the material math in `docs/reference/sheet-metal-material-calculator.html` (functions `nest`, `costAt`, `computePart`). In `packages/calc` define the types `ShopConfig`, `MaterialRow`, `ProcessPreset`, `PartInput`, `QuoteInput`, `CostStack`, `QuoteResult` (document every field with a JSDoc line and its unit). Implement `material.ts`: nesting per §5.1 (clamp on one width edge, kerf added to each part, both orientations, max; plus a `nestForBlankLengths()` that evaluates the blank multiples table and returns yield per length), blank cost, minimum charge, and `materialAtQty()` honoring parity flag Q1. Write unit tests: the prototype's cases (13.38×7.858, 16 ga, 48×120 laser → 42/sheet with 0.375 spacing; 33 per 96-in blank with 0.5 kerf), the golden material numbers from §9 (blank cost 68.83, min 8.6045, 2.0859 per part, 10.3254 at qty 1), a part that doesn't fit, and mm inputs. No I/O, no DB, no console output.

✅ `npm test -w packages/calc` green; `material.test.ts` asserts the four golden intermediates to 4 decimals.

### 1.3 Calc engine — cutting (laser + punch) ⏱ 1 h
*Prompt to Claude:*
> Read REQUIREMENTS §5.2, §5.3, §6 and the LASER WORKSHEET formulas in `docs/discovery.md` / the xls. Implement `laser.ts` (feature list → cut inches, pierces, hours per 100 with the 1.08 loss factor, pallet-change rule) and `punch.ts` (hit counter → hours per 100). Inputs are explicit numbers, never looked up from the DB inside calc. Tests: the golden laser case (perimeter 58 in, 1 pierce, 1 intersection, 220 in/min, 0.1 s pierce, 33 per blank → 0.52255 h/100), zero-feature part, a 200-hole part, a punch part with 40 bridges + 4 embosses.

✅ Golden laser hours match to 5 decimals.

### 1.4 Calc engine — operations, finish, roll-up ⏱ 1.5 h
*Prompt to Claude:*
> Read REQUIREMENTS §5.4–§5.7 and §9. Implement `operations.ts` (fixed $ and direct $ per 100, with the ×60/×100 rule behind `parity.machineOpsFactor60`), `finish.ts` (plating MAX rule; coating model with `parity.legacyCoatingModel`; silkscreen), `hardware.ts`, `nre.ts`, and `rollup.ts` implementing §5.6 exactly, returning a `CostStack` per quantity break with every intermediate (so the UI can show the stack, not just totals) and `materialPctOfSelling`. Then write `test/golden.test.ts` that loads `fixtures/golden-workbook.json`, builds a `ShopConfig` from the seed JSON, runs `computeQuote()`, and asserts all six selling prices within ±0.005 and the six material % values within ±0.001. If any number is off, do NOT tweak the fixture — trace the formula in the xls and fix the engine; explain the discrepancy in the commit message.
...
*Prompt to Claude:*
> Read CLAUDE.md, REQUIREMENTS §3 and §7. In `packages/db` define the Drizzle schema for SQLite: shops, users, sessions, materials, material_prices (versioned), process_presets, operations, plating_specs, coating_models, silkscreen_tiers, assembly_standards, customers, parts, quotes, quote_lines, quote_versions (JSON snapshot of ShopConfig + inputs + results), attachments, audit_log. ULID primary keys, `created_at/updated_at/archived_at` everywhere, foreign keys on. Generate the initial migration. Write `seed.ts` that loads `seed/*.json` from Task 0.2 into a fresh DB and creates an `admin` user from env. Add `npm run db:migrate` and `npm run db:seed`.
...
What changes for another sheet metal shop: `config export/import` (their rates), the seed script (their workbook, if any), logo/terms, and parity flags. What should not change: `packages/calc`, the UI, deploy. For a shop in a different trade (machining, millwork) the material module and operations catalog are reused; cutting/finish modules are swapped — that is why calc is split by module.
